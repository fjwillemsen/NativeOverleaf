...
internalurls="--internal-urls .*?(login|profile|engine|auth|idp|identity|secure|sso|account.*)\..*?(?<TLD>\.\w+?)(?:$|\/)"   # matches all *(login|profile|engine|auth|account.*|etc...).*.<top-level-domain> URLs until the first forward-slash, may be a too greedy because it will also match slugs (e.g. domain.com/login.file.html)
